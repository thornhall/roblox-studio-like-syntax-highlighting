...
const getServiceCompletionProvider_1 = require("./providers/getServiceCompletionProvider");
...
function createClassAutoComplete() {
    const autoCompleteClass = vscode.commands.registerCommand("roblox.autoCompleteClass", async (classCapture) => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
        const doc = editor.document;
...
function validateParentDirectory(fsPath) {
...
function getFileExtensionForNewFile(fsPath) {
    const serverLua = [
        ".server.lua",
        "ServerStorage",
        "ServerScriptService"
...
function getDirectoryPath(fsPath) {
    try {
        const stats = fs.statSync(fsPath);
...
function getCustomExtension(filePath) {
...
function createDuplicateScriptCommand() {
    const createDuplicateModuleScript = vscode.commands.registerCommand("roblox.duplicateRobloxScript", async (uri) => {
        if (!uri || !uri.fsPath) {
            return;
        }
...
function activate(context) {
    ...
    const provider = vscode.languages.registerCompletionItemProvider({ language: "luau" }, new getServiceCompletionProvider_1.GetServiceCompletionProvider(), '"', // Trigger on double quote
